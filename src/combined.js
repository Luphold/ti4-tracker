...
// ── Join room ─────────────────────────────────────────────────────────────
async function joinRoom(code, admin, callbacks = {}) {
  stopPolling();
...
async function broadcastVictory(gameState) {
  if (!gameState) return;
  await persistState(gameState);
  if (channel && connected && isAdmin) {
...
async function fetchLatestState(code) {
...
async function saveCompletedGame(gameState) {
  if (!supabase) return;
  try {
...
function defaultPlayer(name = '', faction = FACTIONS[0], color = '#c39bd3') {
  return {
    name,
    faction,
    color,
...
function initGame(isAdmin, onUpdate) {
  _isAdmin = isAdmin;
  _onUpdate = onUpdate;
}

function getState() { return _state; }
function getIsAdmin() { return _isAdmin; }
...
async function setVP(playerIdx, delta) {
...
async function setVPDirect(playerIdx, value) {
...
async function removePlayer(playerIdx) {
  await commit({ players: _state.players.filter((_, i) => i !== playerIdx) });
}

async function setVpGoal(goal) {
  await commit({ vpGoal: goal });
}

async function endGame(winnerName = null) {
  const t = { ..._state.timer };
  if (!t.paused) {
...
function formatMs(ms) {
  ...
  window.broadcastVictory = broadcastVictory;
  window.fetchLatestState = fetchLatestState;
  window.saveCompletedGame = saveCompletedGame;
  ...
  window.getHistoryEntry = getHistoryEntry;
  ...
  window.STARTING_TECH = STARTING_TECH;
  window.FACTION_DATA = FACTION_DATA;
  window.STRATEGY_CARDS = STRATEGY_CARDS;
  window.STAGE1 = STAGE1;
  window.STAGE2 = STAGE2;
  window.SECRET_OBJECTIVES = SECRET_OBJECTIVES;
  window.TYPE_ORDER = TYPE_ORDER;
  window.SOURCE_LABEL = SOURCE_LABEL;
  window.DIFFICULTY_COL = DIFFICULTY_COL;
  window.COLOR_LABELS = COLOR_LABELS;
  window.STATUS_CHECKLIST = STATUS_CHECKLIST;
  window.LAWS = LAWS;
})();
